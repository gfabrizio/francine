...
class TinyDBRepository(BaseRepostitory):
    ...
    def add(self, task: Task):
        self.db.insert({'description': task.description, 'timestamp': task.timestamp.isoformat()})

    def list(self):
        return [Task(item['description'], datetime.fromisoformat(item['timestamp'])) for item in self.db.all()]
    
    def get(self):
        return [Task(item['description'], datetime.fromisoformat(item['timestamp'])) for item in [self.db.all()[0]]]
